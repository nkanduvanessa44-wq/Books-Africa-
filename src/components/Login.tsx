...
import { signInWithEmailAndPassword, getRedirectResult, signInWithPopup, GoogleAuthProvider } from 'firebase/auth';
import { doc, getDoc, setDoc } from 'firebase/firestore';
import { auth, db } from '../lib/firebase';
import { handleFirestoreError, OperationType } from '../lib/firestore-errors';
import { BookOpen, Chrome, KeyRound, ExternalLink, ShieldAlert, CheckCircle2 } from 'lucide-react';
  ...
  useEffect(() => {
    const checkConnection = async () => {
      try {
        ...
        console.warn('Initial connection check:', err);
  ...
  const handleLogin = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (err.code === 'auth/operation-not-allowed') {
        ...
        setError('Email/Password sign-in is disabled in Firebase. Use Google Login or enable it in Firebase Console.');
      } else {
      ...
      try {
        ...
        if (!userDoc.exists()) {
          await setDoc(doc(db, 'users', user.uid), {
            ...
            email: user.email || '', // Ensure it's a string, even if empty
            role: 'customer', // Default to reader
    ...
    <div className="min-h-screen flex items-center justify-center p-4 bg-gradient-to-br from-amber-500/10 via-emerald-900/10 to-teal-900/20 backdrop-blur-3xl">
      <div className="w-full max-w-md bg-white/75 backdrop-blur-2xl rounded-[40px] shadow-2xl p-8 md:p-10 border border-white/60 relative overflow-hidden">
        ...
        <div className="flex flex-col items-center mb-8 relative z-10">
          <div className="w-20 h-20 bg-gradient-to-br from-primary/20 to-emerald-600/20 rounded-[28px] flex items-center justify-center mb-4 border border-white/80 shadow-lg shadow-primary/10">
            <BookOpen className="w-10 h-10 text-primary" />
          </div>
          <h2 className="text-3xl font-serif font-black italic tracking-tight text-primary">Books Africa</h2>
          <p className="text-accent text-xs font-black uppercase tracking-widest mt-1">Sign in to your account</p>
          {connectionOk === false && (
            <div className="mt-3 flex items-center gap-1.5 text-red-600 text-[10px] font-black uppercase tracking-widest bg-red-500/10 px-3 py-1 rounded-full border border-red-500/20 backdrop-blur-md">
              <span className="w-1.5 h-1.5 bg-red-500 rounded-full animate-pulse" />
              Offline / Server Connection Issue
            </div>
          )}
          {connectionOk === true && (
            <div className="mt-3 flex items-center gap-1.5 text-emerald-600 text-[10px] font-black uppercase tracking-widest bg-emerald-500/10 px-3 py-1 rounded-full border border-emerald-500/20 backdrop-blur-md">
          ...
          <div className="mb-6 p-5 bg-white/80 backdrop-blur-md border border-primary/20 rounded-[28px] text-xs text-natural-text space-y-3.5 shadow-sm">
            ...
            <div className="pt-2 border-t border-primary/10">
              ...
              <p className="mt-1 text-accent/80 font-sans">
                You can sign in instantly using the <strong>Google Login</strong> option below without any project configuration.
              </p>
            </div>
          </div>
        )}
        ...
        <form onSubmit={handleLogin} className="space-y-4 relative z-10">
          <div>
            <label className="block text-[10px] uppercase font-black tracking-widest text-accent mb-2">Email Address</label>
            <input
              type="email"
              required
          ...
          <button
            type="submit"
            disabled={loading}
            className="w-full bg-primary hover:bg-primary-dark text-white font-black text-xs uppercase tracking-[0.2em] py-4.5 rounded-2xl transition transform active:scale-95 disabled:opacity-50 shadow-xl shadow-primary/20"
          >
          ...
          <div className="relative my-6">
            ...
            <div className="relative flex justify-center text-[10px] uppercase font-black tracking-widest">
              <span className="px-4 bg-white/80 backdrop-blur-md text-accent/60 rounded-full border border-white">Or Secure Connect</span>
            </div>
          </div>
